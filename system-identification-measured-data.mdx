...
```
+-------+     +--------+     +----------+     +----------+     +-----------+
| Apply |     | Record |     | Choose   |     | Estimate |     | Validate  |
| step  |---->| output |---->| model    |---->| params   |---->| on new    |
| input |     | data   |     | structure|     | (fit)    |     | data      |
+-------+     +--------+     +----------+     +----------+     +-----------+
                                  |                |
                            First-order?     Least squares
                            Second-order?    Nonlinear opt
                            With delay?      Gradient descent
```
